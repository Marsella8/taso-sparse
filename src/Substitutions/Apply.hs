...
applyMatchedSubstitution target sub match = do
  ...
  let srcGraph = subSrc sub
      ...
      contextGraph = graphWithoutKeys srcImage target
  guard (Set.null (deletedImage `Set.intersection` graphRefs contextGraph))
  let dstInstMap = Map.fromList
      ...
      boundaryRename = Map.fromList
        (zip (Set.toAscList aliasedBoundary) (freshTensors used))
      boundaryTensors = inputImage Set.\\ graphTensorVars contextGraph
      boundaryGraph = graphRename boundaryRename (graphRestrictKeys boundaryTensors target)
      dstCore = graphWithoutKeys (graphInputs instDst) instDst
      inputRename = Map.fromList
        [ (dt, Map.findWithDefault tgt tgt boundaryRename)
        | (st, dt) <- Bi.toList (subInputMap sub)
        , let tgt = mt match st
        ]
      outputRename = Map.fromList
        [ (dt, mt match st) | (st, dt) <- Bi.toList (subOutputMap sub) ]
