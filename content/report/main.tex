...
In several modern systems, these substitutions are hand-written. TASO showed that many useful substitutions can instead be discovered automatically by enumerating candidate graph pairs and then formally verifying the potentially valid ones using a smaller set of base operator axioms. In this paper, we study the converse question: once such substitutions have been discovered, are they already derivable as explicit compositions of the base TASO axioms inside a rewrite system? 

To answer this question, we implement an explicit graph rewrite engine over a symbolic graph IR, together with bounded search for explicit derivations and derivation-trace reconstruction. Evaluated on 444 non-isomorphic substitutions extracted from the TASO implementation, our system recovers over $97\%$ of them, with most derivations requiring only a small number of axiom applications. These results suggest that the large majority of TASO's substitutions are not independent primitive substitutions, but derived consequences of a comparatively small axiom basis.
\end{abstract}
\maketitle
...
A substitution consists of a source graph, a destination graph, and explicit correspondences between the two sides: a bijection between input tensors, a bijection between symbolic non-tensor variables, and a mapping between outputs. We encode TASO's base axioms in exactly this format, choosing one side as the source and the other as the destination; when an axiom is invertible, the reverse orientation is also available during search. Figure~\ref{fig:axiom-matmul-factor} shows one example substitution in our representation.
...
For a target TASO substitution, our goal is to find an explicit directed derivation from one side to the other. In practice we try both orientations: first a derivation from \(L\) to \(R\), and if that is not found within the search budget, a derivation from \(R\) to \(L\). A naive forward breadth-first search for either orientation quickly becomes infeasible, since the number of reachable graphs grows rapidly with depth.
...
In addition to the base axioms, our search uses a small set of derived lemmas. These are not introduced as new semantic primitives, but as proof-shortening rules that make long derivations reachable under a bounded search budget. This is consistent with TASO itself, which also introduces lemma-style shortcuts during verification for some cases.

\subsection{Canonicalization}

During search, many generated graphs differ only by tensor naming or by redundant intermediate bindings. To avoid exploring the same state repeatedly, we canonicalize every generated graph before inserting it into the search frontier. Canonicalization performs common subexpression elimination, removes bindings not reachable from the outputs, and then renames the remaining tensors deterministically. Search therefore compares graphs by their canonical forms rather than by their original names. This is especially effective in our setting because the graph representation has ordered inputs at each operator, so canonicalization does not need to solve a general unlabeled graph isomorphism problem.
...
We evaluated our system on 444 non-isomorphic substitutions extracted from the TASO implementation. A substitution was counted as recovered if our bounded search found an explicit left-to-right or right-to-left derivation trace connecting its two sides; bidirectional frontier expansion was used only to accelerate that search. Throughout these experiments, we used \texttt{maxDepth = 5} and \texttt{maxNumSteps = 12000}.
...
Table~\ref{tab:deriv-stats} summarizes the distribution of derivation lengths. Most recovered substitutions required only two substitution steps, and only a small number required more than four. This indicates that, for most of the substitution set, the axioms do not merely imply the desired substitutions in principle, but do so through relatively short derivations.
...
Our results support two main conclusions. First, the large majority of TASO's discovered substitutions appear to be structurally derivable from a comparatively small axiom basis, and therefore behave more like derived lemmas than unrelated primitive facts. Second, these derivations are usually short. Most recovered substitutions require only a small number of steps (once similar lemmas to those used in TASO proofs are utilised) and only two recovered cases reach length six. This matters because it suggests that the base axioms are not merely expressive enough in principle, but already close to the discovered substitution set in rewrite ``distance''.
...
This example highlights an important aspect of our task. Our goal is not merely to show that two graphs have a common reduct, but to recover a usable directional rewrite trace. For substitutions involving non-invertible, information-losing rules, these two notions can come apart. 
